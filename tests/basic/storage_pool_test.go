...
func CreateNewPoolsOnMultipleNodesInParallel(nodes []node.Node) error {
	...
	wg.Add(numGoroutines)
	var poolList map[string]int
	var poolListAfterCreate map[string]int

	for _, eachNode := range nodes {
		poolList[eachNode.Name] = len(eachNode.StoragePools)
	...
	for pool, poolCount := range poolList {
		if poolList[pool] <= poolListAfterCreate[pool] && poolCount < 8 {
	...
	*/
	...
	It(stepLog, func() {
		...
		for _, each := range getNodes {
			if IsLocalCluster(each) {
				log.FailOnError(fmt.Errorf("Test specific to cloud drives, cannot be run on local cluster"), "error adding cloud drives in parallel")
			}
		}

		err := CreateNewPoolsOnMultipleNodesInParallel(getNodes)
