	...
	It("Creating Backup Location with Encryption Keys", func() {
		...
		Step("Creating backup location and cloud setting", func() {
			...
			for _, provider := range providers {
				...
				err = CreateBackupLocation(provider, backupLocationName, backupLocationUID, cloudCredName, cloudCredUID, getGlobalBucketName(provider), orgID, encryptionKey, true)
	...
	It("Change replica while restoring backup", func() {
		...
		Step("Taking backup of applications", func() {
			...
			for _, namespace := range bkpNamespaces {
				...
				err = CreateBackupWithValidation(ctx, backupName, SourceClusterName, backupLocation, backupLocationUID, appContextsToBackup, labelSelectors, orgID, clusterUid, "", "", "", "")
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creation and Validation of backup [%s]", backupName))
		...
		Step("Restoring the backed up application", func() {
			...
			for i, backupName := range backupNames {
				...
				err = CreateRestoreWithValidation(ctx, restoreName, backupName, namespaceMapping, storageClassMapping, SourceClusterName, orgID, appContextsToBackup)
	...
	It("Resize after the volume is restored from a backup", func() {
		...
		Step("Creating backup location", func() {
			...
			for _, provider := range providers {
				...
				BackupLocationUID = uuid.New()
				BackupLocationMap[BackupLocationUID] = backupLocation
				err := CreateBackupLocation(provider, backupLocation, BackupLocationUID, credName, CloudCredUID, getGlobalBucketName(provider), orgID, "", true)
...
var _ = Describe("{RestoreEncryptedAndNonEncryptedBackups}", func() {
	...
	It("Restore encrypted and non encrypted backups", func() {
		...
		err = CreateBackupLocation(providers[0], backupLocationNames[0], BackupLocationUID, CredName, CloudCredUID, getGlobalBucketName(providers[0]), orgID, "", true)
		...
		err = CreateBackupLocation(providers[0], backupLocationNames[1], BackupLocation1UID, CredName, CloudCredUID, encryptionBucketName, orgID, encryptionKey, true)
		...
		for _, namespace := range appNamespaces {
			for backupLocationUID, backupLocationName := range backupLocationMap {
				...
				Step("Restoring application from first schedule backup", func() {
					...
					restoreNames = append(restoreNames, restoreName)
					err = CreateRestoreWithValidation(ctx, restoreName, firstScheduleBackupName, make(map[string]string), make(map[string]string), destinationClusterName, orgID, appContextsToBackup)
				...
				Step("Restoring application from recent backup", func() {
					...
					restoreNames = append(restoreNames, restoreName)
					err = CreateRestoreWithValidation(ctx, restoreName, nextScheduleBackupName, make(map[string]string), make(map[string]string), destinationClusterName, orgID, appContextsToBackup)
