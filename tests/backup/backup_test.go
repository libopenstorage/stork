	...
	It("Share all backups at cluster level with a user group and revoke it and validate", func() {
		...
		Step("Adding Credentials and Registering Backup Location", func() {
			...
			for _, provider := range providers {
				...
				err := CreateBackupLocation(provider, customBackupLocationName, backupLocationUID, credName, cloudCredUID, bucketName, orgID, "")
				dash.VerifyFatal(err, nil, "Creating backup location")
				customBackupLocationName = fmt.Sprintf("autogenerated-backup-location-%v", timestamp)
...
func GetbackupScheduleUID(backupName string, orgID string, ctx context.Context) (string, error) {
	backupDriver := Inst().Backup
	...
	log.InfoD("response here is  %v", resp)
	log.InfoD("resp.GetBackupSchedule().GetUid() - %v ", resp.GetBackupSchedule().GetUid())
	scheduleBackupyUid := resp.GetBackupSchedule().GetUid()

	return scheduleBackupyUid, err
...
var _ = Describe("{ClusterBackupShareToggle}", func() {
	...
	It("Validate after toggling the access, user can perform operation on new backup", func() {
		...
		Step("Create Schedule Backup", func() {
			backupName = fmt.Sprintf("%s-%v", BackupNamePrefix, time.Now().Unix())
			timestamp := strconv.Itoa(int(time.Now().Unix()))
			periodicPolicyName = fmt.Sprintf("%s-%s", "interval", timestamp)
			log.InfoD("SCHEDULE_POLICY NAME %s", periodicPolicyName)
			log.InfoD("Creating backup interval schedule policy")
			intervalSchedulePolicyInfo := Inst().Backup.CreateIntervalSchedulePolicy(5, 15, 2)
			...
			//prerule
			preRuleStatus, preRuleName, err := Inst().Backup.CreateRuleForBackup(bkpNamespaces[0], orgID, "pre")
			log.FailOnError(err, "Creating pre rule for deployed apps failed")
			dash.VerifyFatal(preRuleStatus, true, "Verifying pre rule for backup")

			//postrule
			postRuleStatus, postRuleName, err := Inst().Backup.CreateRuleForBackup(bkpNamespaces[0], orgID, "post")
			log.FailOnError(err, "Creating post rule for deployed apps failed")
			dash.VerifyFatal(postRuleStatus, true, "Verifying Post rule for backup")
			...
			log.FailOnError(err, "Generating post rule UID for deployed apps failed")

			log.InfoD("Fetching Schedule Backup uid")
			schPolicyUid, err = Inst().Backup.GetSchedulePolicyUid(orgID, ctx, periodicPolicyName)
			log.FailOnError(err, "Fetching schedule policy failed")

			//CreateSchedule backup
			log.InfoD("backup schedule name - %v", backupName)
			err = CreateScheduleBackup(backupName, SourceClusterName, backupLocationName, backupLocationUID, []string{bkpNamespaces[0]}, nil, orgID, preRuleName, preRuleUid, postRuleName, postRuleUid, periodicPolicyName, schPolicyUid, ctx)
			log.FailOnError(err, "Generating Schedule Backup")
		})
		...
		Step("Validate the Access toggle ", func() {
			...
			for _, accessLevel := range accesses {
				...
				log.InfoD("Current Access is  %v", accessLevel)
				...
				log.InfoD("len(userBackups %v", len(userBackups))
				log.FailOnError(err, "Fail on Fetching backups for %s", userName)

				restoreName := fmt.Sprintf("%s-%v", RestoreNamePrefix, time.Now().Unix())
				ValidateSharedBackupWithUsers(userName, ctxNonAdmin, accessLevel, userBackups[len(userBackups)-1], restoreName)
				if accessLevel != ViewOnlyAccess {
					restoreNames = append(restoreNames, restoreName)
				}
				log.InfoD("RestoreNames - %v", restoreNames)
				if accessLevel == FullAccess {
					log.InfoD("The fullaccess exit begins ")
					break
				}
				//wait for 15 mins
				log.InfoD("wait for 15 mins")
				...
				log.InfoD("recent backup - %v ", recentBackupName)
	...
	JustAfterEach(func() {
		...
		//Get scheduleBackupUid
		log.InfoD("Get scheduleBackupUid")
		scheduleBackupyUid, err := GetbackupScheduleUID(backupName, orgID, ctx)
		log.InfoD("scheduleBackupyUid - %v", scheduleBackupyUid)
		log.FailOnError(err, "Error deleting Schedule backup %v", backupName)

		//Delete Schedule Backup-
		log.InfoD("Delete Schedule Backup-")
		err = DeleteScheduleBackup(backupName, scheduleBackupyUid, periodicPolicyName, schPolicyUid, orgID)
		log.FailOnError(err, "Error deleting Schedule backup %v", backupName)
		...
		//Delete restore for user
		log.InfoD("Deleting restore for user")
		ctxNonAdmin, err := backup.GetNonAdminCtx(userName, password)
		log.FailOnError(err, "Fetching %s ctx", userName)

		for _, restore := range restoreNames {
			DeleteRestore(restore, orgID, ctxNonAdmin)
		}
...
var _ = Describe("{BackupLocationWithEncryptionKey}", func() {
	...
	It("Creating cloud account and backup location", func() {
		...
		err := CreateBackupLocation(providers[0], backupLocationName, BackupLocationUID, cloudCredName, CloudCredUID, bucketName, orgID, encryptionKey)
		dash.VerifyFatal(err, nil, "Creating backup location")
		log.InfoD("Deploy applications")
		contexts = make([]*scheduler.Context, 0)
		for i := 0; i < Inst().GlobalScaleFactor; i++ {
...
var _ = Describe("{RestoreEncryptedAndNonEncryptedBackups}", func() {
	...
	It("Creating bucket, encrypted and non-encrypted backup location", func() {
		...
		err := CreateBackupLocation(providers[0], backupLocationNames[0], BackupLocationUID, CredName, CloudCredUID, bucketName, orgID, "")
		dash.VerifyFatal(err, nil, "Creating backup location")
...
var _ = Describe("{ShareAndRemoveBackupLocation}", func() {
	...
	It("Share and remove backup location and add it back and check from other users if they show up", func() {
		...
		Step("Adding new backup location to the cluster", func() {
			...
			for _, provider := range providers {
				bucketName := fmt.Sprintf("%s-%s", provider, bucketNames[0])
				newBkpLocationName = fmt.Sprintf("new-%s-%s-bl", provider, bucketNames[0])
				newBackupLocationUID = uuid.New()
				newBackupLocationMap[newBackupLocationUID] = newBkpLocationName
				err := CreateBackupLocation(provider, newBkpLocationName, newBackupLocationUID, cloudCredName, cloudCredUID, bucketName, orgID, "")
				dash.VerifyFatal(err, nil, "Creating backup location")
			}
		})
...
var _ = Describe("{IssueMultipleRestoresWithNamespaceAndStorageClassMapping}", func() {
	...
	It("Issue multiple restores with namespace and storage class mapping", func() {
		...
		Step("Create new storage class on source and destination cluster for storage class mapping for restore", func() {
			...
			scName = fmt.Sprintf("replica-sc-%v", timestamp)
			params["repl"] = "2"
			v1obj := meta_v1.ObjectMeta{
				Name: scName,
			}
		...
		Step("Creating backup location and cloud setting", func() {
			...
			for _, provider := range providers {
				bucketName := fmt.Sprintf("%s-%s", provider, bucketNames[0])
				cloudCredName = fmt.Sprintf("%s-%s-%v", "cred", provider, timestamp)
				bkpLocationName = fmt.Sprintf("%s-%s-%v-bl", provider, bucketNames[0], timestamp)
				cloudCredUID = uuid.New()
				backupLocationUID = uuid.New()
				backupLocationMap[backupLocationUID] = bkpLocationName
				CreateCloudCredential(provider, cloudCredName, cloudCredUID, orgID)
