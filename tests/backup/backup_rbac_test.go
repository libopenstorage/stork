...
var _ = Describe("{VerifyRBACForPxAdmin}", func() {
	...
	It("Validates the RBAC operation for px-admin group user.", func() {
		...
		Step("Verify Px-Admin group user has permission to create cloud credential  and backup location", func() {
			...
			for _, provider := range providers {
				adminCloudCredentialName = fmt.Sprintf("autogenerated-cred-%v", time.Now().Unix())
				...
				adminBackupLocationName = fmt.Sprintf("autogenerated-backup-location-%v", time.Now().Unix())
		...
		Step(fmt.Sprintf("Verify Px-Admin User has permission to create a schedule policy"), func() {
			log.InfoD("Verify Px-AdminUser has permission to create a schedule policy")
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "failed to fetch px-admin ctx")
		...
		Step("Verify px-admin User has permission to share RBAC resources with non-admin users", func() {
			...
			dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying updation of owbership for backuplocation - %s", adminBackupLocationName))
		...
		Step(fmt.Sprintf("Delete px-admin backups from the px-admin context"), func() {
			log.InfoD(fmt.Sprintf("Deleting px-admin backups from the px-admin context"))
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "failed to fetch px-admin ctx")
		...
		Step(fmt.Sprintf("Delete px-admin restores from the px-admin context"), func() {
			log.InfoD(fmt.Sprintf("Deleting  px-admin restores from the px-admin context"))
			ctx, err := backup.GetAdminCtxFromSecret()
			log.FailOnError(err, "failed to fetch px-admin ctx")
