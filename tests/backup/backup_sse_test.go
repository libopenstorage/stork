...
var _ = Describe("{CreateBackupAndRestoreForAllCombinationsOfSSES3AndDenyPolicy}", Label(TestCaseLabelsMap[CreateBackupAndRestoreForAllCombinationsOfSSES3AndDenyPolicy]...), func() {
	...
	It("Create backup and restore for all combinations of SSE-S3 form px-backup side and deny policy configured on AWS S3 bucket", func() {
		...
		for _, provider := range providers {
			...
			Step("Create BackupLocation with bucketWithPolicy and SSE set to false", func() {
				...
				err := CreateCloudCredential(provider, credNameForBlWithoutSse, cloudCredUidForBlWithoutSse, BackupOrgID, ctx)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Verifying creation of cloud credential named [%s] for org [%s] with [%s] as provider", credNameForBlWithoutSse, BackupOrgID, provider))
				...
				err = CreateBackupLocation(provider, backupLocationWithoutSse, backupLocationUidWithoutSse, credNameForBlWithoutSse, cloudCredUidForBlWithoutSse, customBucketsWithDenyPolicy[0], BackupOrgID, "", true)
			...
			Step("Create BackupLocation with bucketWithOutPolicy and SSE set to false", func() {
				...
				log.InfoD("Created Cloud Credentials with name - %s", credNameForBlWithoutSse)
				backupLocationWithoutSse = fmt.Sprintf("autogenerated-backup-location-%v", time.Now().Unix())
				err = CreateBackupLocation(provider, backupLocationWithoutSse, backupLocationUidWithoutSse, credNameForBlWithoutSse, cloudCredUidForBlWithoutSse, customBucketWithOutPolicy, BackupOrgID, "", true)
			...
			Step("Create BackupLocation with bucketWithPolicy and SSE set to true", func() {
				...
				err = CreateS3BackupLocation(customBackupLocationWithSse, backupLocationUID, credName, cloudCredUID, customBucketsWithDenyPolicy[0], BackupOrgID, "", true, api.S3Config_SSE_S3)
			...
			Step("Create and validate restore with replace policy set to retain", func() {
				...
				err = CreateRestoreWithReplacePolicy(restoreName, backupsWithSse[0], make(map[string]string), SourceClusterName, BackupOrgID, ctx, make(map[string]string), 2)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creating restore [%s]", restoreName))
				restoreList = append(restoreList, restoreName)
				backupNamespace, err := FetchNamespacesFromBackup(ctx, backupsWithSse[0], BackupOrgID)
				...
				err = ValidateRestore(ctx, restoreName, BackupOrgID, expectedRestoredAppContextListNew, make([]string, 0))
			...
			Step("Validating restores with sc and ns mapping", func() {
				...
				for restoreName, mapNsSc := range restoreNsScMap {
					...
					go func(restoreName string, mapNsSc []map[string]string) {
						...
						err = ValidateRestore(ctx, restoreName, BackupOrgID, expectedRestoredAppContextList, make([]string, 0))
						if err != nil {
							errChan <- err
						}
						mutex.Unlock()
			...
			Step("Update backup location backupLocationUidWithoutSse to sse true", func() {
				...
				err = UpdateBackupLocation(provider, backupLocationWithoutSse, backupLocationUidWithoutSse, BackupOrgID, credNameForBlWithoutSse, cloudCredUidForBlWithoutSse, ctx, api.S3Config_SSE_S3)
			...
			Step("Create BackupLocation with bucketWithPolicy and SSE set to true post px-backup and stork restart", func() {
				...
				err = CreateS3BackupLocation(newBackupLocationWithSseAfterRestart, backupLocationUID, credName, cloudCredUID, customBucketsWithDenyPolicy[1], BackupOrgID, "", true, api.S3Config_SSE_S3)
			...
			Step("Create restore with backup taken on new BackupLocation created post px-backup and stork restart", func() {
				...
				err = CreateRestoreWithReplacePolicy(restoreName, backupNameAfterPxBackupRestart, make(map[string]string), SourceClusterName, BackupOrgID, ctx, make(map[string]string), 2)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creating restore [%s]", restoreName))
				restoreList = append(restoreList, restoreName)
				backupNamespace, err := FetchNamespacesFromBackup(ctx, backupNameAfterPxBackupRestart, BackupOrgID)
				...
				appContextsToBackup := FilterAppContextsByNamespace(scheduledAppContexts, backupNamespace)
				err = ValidateRestore(ctx, restoreName, BackupOrgID, appContextsToBackup, make([]string, 0))
			...
			Step("Create restore with replace policy set to retain after removal of deny policy", func() {
				...
				err = CreateRestoreWithReplacePolicy(restoreName, backupNameAfterRemovalOfDenyPolicy, make(map[string]string), SourceClusterName, BackupOrgID, ctx, make(map[string]string), 2)
				dash.VerifyFatal(err, nil, fmt.Sprintf("Creating restore [%s]", restoreName))
				restoreList = append(restoreList, restoreName)
				backupNamespace, err := FetchNamespacesFromBackup(ctx, backupNameAfterRemovalOfDenyPolicy, BackupOrgID)
