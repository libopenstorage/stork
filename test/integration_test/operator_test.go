...
var (
	...
	appNameElastic        = "elastic"
	appPathElastic        = "/stork-specs/elasticcr.yaml"
)
...
func TestOperatorMig(t *testing.T) {
	...
	t.Run("testElasticMig", testElasticMig)
}
...
func testElasticMig(t *testing.T) {
	// reset mock time before running any tests
	err := setMockTime(nil)
	require.NoError(t, err, "Error resetting mock time")

	validateAndDestroyCrMigration(t, appNameElastic, appPathElastic)
}
