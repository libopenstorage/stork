...
func deploymentMigrationBackupTest(t *testing.T) {
	for location, secret := range allConfigMap {
		...
		srcBackupLocation, err = k8s.Instance().UpdateBackupLocation(srcBackupLocation)
		...
		err = k8s.Instance().DeleteBackupLocation(srcBackupLocation.Name, srcBackupLocation.Namespace)
