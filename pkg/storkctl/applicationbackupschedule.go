	...
	getApplicationBackupScheduleCommand := &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if len(args) > 0 {
				...
				for _, ns := range namespaces {
					applicationBackupSchedules, err = storkops.Instance().ListApplicationBackupSchedules(ns, v1.ListOptions{})
					if err != nil {
						util.CheckErr(err)
						return
					}
	...
	deleteApplicationBackupScheduleCommand := &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if len(backupLocation) == 0 {
				...
				applicationBackupScheduleList, err := storkops.Instance().ListApplicationBackupSchedules(cmdFactory.GetNamespace(), v1.ListOptions{})
				if err != nil {
					util.CheckErr(err)
					return
				}
...
func getApplicationBackupSchedules(backupLocation string, args []string, namespace string) ([]*storkv1.ApplicationBackupSchedule, error) {
	...
	if len(backupLocation) == 0 {
		...
		applicationBackupScheduleList, err := storkops.Instance().ListApplicationBackupSchedules(namespace, v1.ListOptions{})
		if err != nil {
			return nil, err
		}
		for _, applicationBackupSchedule := range applicationBackupScheduleList.Items {
