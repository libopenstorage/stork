...
// List lists all SchedulePolicies in the indexer.
func (s *schedulePolicyLister) List(selector labels.Selector) (ret []*v1alpha1.SchedulePolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.SchedulePolicy))
	})
	return ret, err
}

// Get retrieves the SchedulePolicy from the index for a given name.
func (s *schedulePolicyLister) Get(name string) (*v1alpha1.SchedulePolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("schedulepolicy"), name)
	}
	return obj.(*v1alpha1.SchedulePolicy), nil
}
